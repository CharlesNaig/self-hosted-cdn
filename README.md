...
- `127.0.0.1:3001`: nginx gateway that only permits `GET`/`HEAD /cdn/*`.
...
The Express app remains private. The only future public surface is the nginx gateway's `/cdn/*` read path; it never forwards `/api/*` or write methods. Do not add a Vercel rewrite to the admin API.
